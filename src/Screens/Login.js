...
const Login = () => {
  ...
  const Log = async( )=>{
    // axios.post("https://meven.pythonanywhere.com/api-auth/login/?next=/api/customer/register/", {
    //   firstName:username,
    //   lastName: pass
    // })
  ...
  <div class="form-group" style={{marginBottom:33}}>
    ...
    <input  onChange={(e)=>setUsername(e.target.value)}  style={{height:50,width:460,borderRadius:10,backgroundColor:"#D9D9D9"}} type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter email"/>
    {/* <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> */}
  </div>
  <div class="form-group " style={{marginBottom:33}}>
    <label for="exampleInputPassword1"  className="font">Password</label>
    <input onChange={(e)=>setPass(e.target.value)} style={{height:50,width:460,borderRadius:10,backgroundColor:"#D9D9D9"}} type="password" class="form-control" id="exampleInputPassword1" placeholder="Password"/>
  </div>
  ...
  <button   onClick={()=>Log()}  className="d-flex align-items-center justify-content-center text-center" style={{ height:60 ,width:210 ,borderRadius:18,backgroundColor:"#6AE531"}} type="submit">
